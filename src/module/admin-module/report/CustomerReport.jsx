...
import { updateResponse } from '../../../api/reportAip';
...
const CustomerReport = () => {
  ...
  const fetchReports = async () => {
    try {
      const response = await fetch.get('/customer-reports/GetAll');
      if (response.status === 200) {
        const groupedReports = response.data.data.reduce((acc, report) => {
          const existingReport = acc.find(r => r.reportId === report.reportId);
          if (existingReport) {
            existingReport.customerReportImages.push(...report.customerReportImages);
          } else {
            acc.push({ ...report, customerReportImages: [...report.customerReportImages] });
          }
          return acc;
        }, []);
  ...
  const { mutate } = useMutation({
    mutationFn: (data) => updateResponse(data),
    onSuccess: () => {
      Swal.fire({
        icon: "success",
        text: "Đã gửi phản hồi đến khách hàng"
      });
                  ...
                  <React.Fragment key={report.reportId}>
                    <TableRow>
                      <TableCell>
                        <Box display="flex" alignItems="center">
                          <Avatar
                            src={report.customer?.avatar || null}
                            alt={report.customer?.name || 'Avatar'}
                            sx={{ width: 40, height: 40, mr: 2, border: '1px solid #3f51b5' }}
                          />
                          <Typography variant="body1" sx={{ fontWeight: 'bold', color: '#3f51b5' }}>
                            {report.customer ? report.customer.name : 'N/A'}
                          </Typography>
                        </Box>
                      </TableCell>
                      <TableCell>{report.customerReportType?.type || 'N/A'}</TableCell>
                      <TableCell align="center">
                        <IconButton onClick={() => handleExpandClick(report.reportId)}>
                          <ExpandMoreIcon
                            sx={{
                              transform: expanded === report.reportId ? 'rotate(180deg)' : 'rotate(0deg)',
                    ...
                    <TableRow>
                      <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={4}>
                        <Collapse in={expanded === report.reportId} timeout="auto" unmountOnExit>
                          <Box margin={2}>
                            ...
                            <Box sx={{ mt: 2 }}>
                              ...
                              <Grid container spacing={1}>
                                {report.customerReportImages?.length > 0 ? (
                                  report.customerReportImages.map((img, index) => (
                                    <Grid item key={index}>
                                      <Avatar
                            ...
                            <Box sx={{ mt: 2 }}>
                              ...
                              <TextareaAutosize
                                ...
                                value={report.reportResponse || responses[report.reportId] || ''}
                              ...
                              {!report.reportResponse && (
                                <Stack direction="row" sx={{ justifyContent: "center", mt: 2 }}>
                                  <Button variant="contained" color="primary" onClick={() => handleSendResponse(report.reportId)}>
                                    Gửi phản hồi
                                  </Button>
                                </Stack>
                              )}
